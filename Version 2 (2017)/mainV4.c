...
void loadWorld(char name[])
{
    ...
    sprintf(nName,"worlds//%s//player.txt",name);
    ...
    for(i=0;i<2;i++)
    {
        ...
        while(curLine[j]!=0)
        {
            player[i]+=(curLine[j]-65)*(int)pow(26,j);
            j++;
        }
    }
    ...
    fflush(w);
    fclose(w);
    sprintf(nName,"worlds//%s//world.txt",name);
    w=fopen(nName,"r");
    ...
    for(i=0;i<3;i++)
    {
        for(j=0;j<2;j++)
        {
            for(k=0;k<256;k++)
            {
                fscanf(w,"%s",curLine);
                curChar=0;
                while(curLine[curChar]!=0)
                {
                    if(curLine[curChar]>=97&&curLine[curChar]<=122)
                    {
                        coeff*=26;
                        coeff+=curLine[curChar]-97;
                    }
                    else if(curLine[curChar]>=65&&curLine[curChar]<=90)
                    {
                        if(coeff==0)
                        {
                            coeff=1;
                        ...
                        for(l=1;l<=coeff;l++)
                        {
                            loadedWorld[i][j][curPlace][k]=curLine[curChar]-65;
                            curPlace++;
                        }
                        coeff=0;
                    }
...
void saveWorld(char name[])
{
    ...
    char fName[100];
    sprintf(fName,"worlds//%s",name);
    mkdir(fName,0700);

    sprintf(fName,"worlds//%s//world.txt",name);
    ...
    fflush(w);
    fclose(w);
    sprintf(fName,"worlds//%s//player.txt",name);
    w=fopen(fName,"w");
    ...
    fflush(w);
    fclose(w);
    sprintf(fName,"worlds//%s//inventory.txt",name);
    w=fopen(fName,"w");
...
void deleteWorld(char wName[])
{
    ...
    sprintf(fullName,"worlds//%s//world.txt",wName);
    printf("%i",remove(fullName));
    sprintf(fullName,"worlds//%s//player.txt",wName);
    printf("%i",remove(fullName));
    sprintf(fullName,"worlds//%s//inventory.txt",wName);
    printf("%i",remove(fullName));
    sprintf(fullName,"worlds//%s",wName);
    rmdir(fullName,0700);
